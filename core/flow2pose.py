...
def Flow2Pose(flow_up, depth, calib, flow_gt=None, uncertainty=None, x=60, y=160, h=600, w=960, MAX_DEPTH=10., flag=True):
    ...
    pose, inliers = poselib.estimate_absolute_pose(pts2d, pts3d, camera_dict=camera, 
                                                opt={
    ...
    # 修复：新版 PoseLib 使 而不是 .q
    
    R = torch.tensor(pose.pose.q)  # 将旋转转换为四元数
    T = torch.tensor(pose.pose.t)                   # 位置属性依然叫 .t
