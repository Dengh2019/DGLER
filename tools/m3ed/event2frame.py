...
if __name__ == '__main__':
    ...
    # Safely recursively create multi-level directories, preventing FileNotFoundError
    ...
    for i in tqdm(range(len(ts_map_prophesee_left)-2)):
        ...
        # =========== New addition：Generate voxel5 Grid ===========
        elif args.method == "voxel5":
           
            event_time_image = events_to_voxel_timesync_torch(
                torch.tensor(np.array(event_data['x'][idx_start:idx_cur], dtype=np.float32)), 
                torch.tensor(np.array(event_data['y'][idx_start:idx_cur], dtype=np.float32)), 
                torch.tensor(np.array(event_data['t'][idx_start:idx_cur], dtype=np.float32)), 
                torch.tensor(np.array(event_data['p'][idx_start:idx_cur], dtype=np.float32)), 
                B=5,
            ...
            event_time_image = event_time_image.numpy() # event_time_image shape : (5, H, W)
        # =========================================================
